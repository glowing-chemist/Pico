...
namespace Render
{
    ...
    Sample Transparent_BTDF::sample(Core::Rand::Hammersley_Generator& rand, const Core::BVH::InterpolatedVertex &position, const glm::vec3& V)
    {
        PICO_ASSERT_VALID(position.mNormal);
        PICO_ASSERT_VALID(V);
        ...
        const bool enter_object = glm::dot(view_tangent, H) >= 0.0f;
        const float eta = 1.0f / m_index_of_refraction;// enter_object ? (1.0f / m_index_of_refraction) : m_index_of_refraction;

        // TODO add consideration for total internal refraction
        const glm::vec3 L = glm::normalize(glm::refract(-view_tangent, H * (enter_object ? 1.0f : -1.0f), eta));
        PICO_ASSERT_VALID(L);
